...
PusherChatWidget.prototype._sendChatButtonClicked = function() {
  ...
  //alert("send button clicked"+JSON.stringify(chatInfo))
};

/* @private */
PusherChatWidget.prototype._sendChatMessage = function(data) {
//alert("_sendChatMessage"+ JSON.stringify(data)+this.settings.chatEndPoint)
  var self = this;
  ...
  $.ajax(
    ...
    {
    url: '/chat',//this.settings.chatEndPoint,
    type: 'post',
    data: {
      'chat_info': data
    },
...
PusherChatWidget._createHTML = function(appendTo) {
  ...
  '<div class="pusher-chat-widget">' +
    '<div class="pusher-chat-widget-header">' +
      ...
      '<input type="hidden" name="nickname" value=<%=current_user.name%>>' +
      '<label for="email" title="So we can look up your Gravatar"></label>' +
